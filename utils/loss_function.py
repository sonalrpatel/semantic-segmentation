...
class Semantic_loss_functions(object):
    ...
    def dice_coef(self, y_true, y_pred):
        ...
        return (2. * intersection + backend.epsilon()) / (
                backend.sum(y_true_f) + backend.sum(y_pred_f) + backend.epsilon())
    ...
    def specificity(self, y_true, y_pred):
        true_negatives = backend.sum(
            backend.round(backend.clip((1 - y_true) * (1 - y_pred), 0, 1)))
    ...
    def depth_softmax(self, matrix):
        sigmoid = lambda x: 1 / (1 + backend.exp(-x))
        sigmoided_matrix = sigmoid(matrix)
        softmax_matrix = sigmoided_matrix / backend.sum(sigmoided_matrix, axis=0)
    ...
    def confusion(self, y_true, y_pred):
        smooth = 1
        y_pred_pos = backend.clip(y_pred, 0, 1)
        y_pred_neg = 1 - y_pred_pos
        y_pos = backend.clip(y_true, 0, 1)
        y_neg = 1 - y_pos
    ...
    def true_positive(self, y_true, y_pred):
        ...
        tp = (backend.sum(y_pos * y_pred_pos) + smooth) / (backend.sum(y_pos) + smooth)
        return tp
    ...
    def true_negative(self, y_true, y_pred):
        ...
        y_pos = backend.round(backend.clip(y_true, 0, 1))
        y_neg = 1 - y_pos
        tn = (backend.sum(y_neg * y_pred_neg) + smooth) / (backend.sum(y_neg) + smooth)
        return tn

    def tversky_index(self, y_true, y_pred):
        y_true_pos = backend.flatten(y_true)
        y_pred_pos = backend.flatten(y_pred)
        true_pos = backend.sum(y_true_pos * y_pred_pos)
        false_neg = backend.sum(y_true_pos * (1 - y_pred_pos))
        false_pos = backend.sum((1 - y_true_pos) * y_pred_pos)
        alpha = 0.7
    ...
    def focal_tversky(self, y_true, y_pred):
        ...
        gamma = 0.75
        return backend.pow((1 - pt_1), gamma)
    ...
    def jacard_similarity(self, y_true, y_pred):
        ...
        intersection = backend.sum(y_true_f * y_pred_f)
        union = backend.sum((y_true_f + y_pred_f) - (y_true_f * y_pred_f))
